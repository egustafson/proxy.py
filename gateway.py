...
__description__ = 'HTTP Forwarding Gateway Server in Python'
...
class TCP(object):
    ...
    def run(self):
        try:
            logger.info('Starting HTTP server listening on port %d' % self.port)
...
class ZmqRelay(object):
    """ZMQ <--> TCP socket relay"""

    def __init__(self, tcpsock, zmqconn):
        self.tsock = tcpsock
        self.zconn = zmqconn
...
class TCPForwarder(object):
    """TCP socket that forwards (bi-directionally) to ZMQ"""

    def __init__(self, lhost='0.0.0.0', lport=8899, zhost='0.0.0.0', zport=19797, backlog=100):
        self.lhost   = lhost
        self.lport   = lport
        self.backlog = backlog
        self.zhost   = zhost
    ...
    def run(self):
        try:
            logger.info("Starting ZMQ connector server on {}:{}+1".format(self.zhost, self.zport))
            self.zmqconn = ZmqServer(self.zhost, self.zport)
            logger.info("Starting HTTP server listening on {}:{}".format(self.lhost, self.lport))
            self.socket = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
            self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            ...
            while True:
                conn, addr = self.socket.accept()
                logger.debug("accepted connection {r} from {r}".format(conn, addr))
                client = Client(conn, addr)
                self.handle(client)
        except Exception as e:
            logger.exception("Exception while running the server {r}".format(e))
        finally:
            logger.info("Closing server socket at {}:{}".format(self.lhost, self.lport))
            self.socket.close()

    def handle(self, client, zmqconn):
        relay = ZmqRelay(client, zmqconn)
        logger.debug("Starting ZMQ relay for connection {r}".format(client.conn))
        relay.run()
...
def main():
    ...
    gw_group = parser.add_mutually_exclusive_group(required=True)
    gw_group.add_argument('--gwserver', action='store_true', help='act as a listeneing gateway server')
    gw_group.add_argument('--gwclient', action='store_true', help='act as a connecting gateway client')
    parser.add_argument('--gwhost', default='127.0.0.1', help='Default:  127.0.0.1')
    parser.add_argument('--gwport', default='19797', help='Default: 19797 (+ 19798)')
    ...
    port = int(args.port)
    gw_host = args.gwhost
    gw_port = int(args.gwport)


    try:
        if args.gwserver:
            logger.info("Gateway server, ZMQ listening on ports {{{}, {}}}".format(gw_port, gw_port+1))
            gw = HTTP(hostname, port)
        else:
            logger.info("Gateway client, ZMQ connecting to {}:{{{}, {}}}".format(gw_host, gw_port, gw_port+1))
            gw = HTTP(hostname, port)
